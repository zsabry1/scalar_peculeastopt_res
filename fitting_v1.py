'''Version 1.0 of the astronomy fitting class for Python 2.7.
...
#        self.chi_square_p = []
    ...
    def fit_frac_residuals(self, iterations=1000, yerr_systematic=0, mean=False):
        """A bootstrap least_squares fit using fractional residuals as errors. Can cylce through asymmetric
        ...
        pfit = least_squares(errfunc, self.init, bounds=self.bds, args=(self.datax, self.datay), max_nfev=10000)
        residuals = pfit.fun
        model=self.fitfunc(pfit.x, self.datax)
        
        # Calculating fractional residuals
        frac_res=[]
        for i in range(len(self.datax)):
            frac_res.append(residuals[i]/model[i])

        # Get the stdev of the residuals
        sigma_res = np.std(frac_res)
        ...
        for i in range(iterations):
            ...
            randomdataY = self.datay + randomDelta*self.datay
